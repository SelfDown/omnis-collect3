...
class EventService(CollectService):
    ...
    def before_event(self):
        ...
        for index, to_service in enumerate(to_services):
            ...
            # 添加日志
            self.add_create_log_data(to_service)
            services.append(get_safe_data(self.get_service_name(), to_service))
        if len(services) <= 0:
    ...
    def get_service_item_params(self, service_item):
        return {
    ...
    def add_create_log_data(self, service_item):
        ...
        params = self.get_service_item_params(service_item)
        log_create = get_safe_data(self.get_log_create_name(), template_log)
        service = {self.get_service_name(): log_create}
        ...
        if not self.is_success(create_item):
            ...
            return False
        self.create_log_list.append(self.get_data(create_item))

    def create_log_data(self):
        """
        执行日志添加服务
        """
        self.log(self.create_log_list)

    def add_update_log_data(self):
        """
        添加更新日志
        """
